...
from .beam_polar import BeamPolar_MOD
...
class BeamGrid_MOD:
    ...
    def __init__(self, filepath00, filepath45):
        """Initialize the BeamGrid object."""

        super().__init__()
        self.filepath00 = filepath00
        self.filename00 = filepath00.split("/")[-1]
        self.filepath45 = filepath45
        self.filename45 = filepath45.split("/")[-1]
        self.__post_init__()

    def __post_init__(self):
        """Read and parse the beam grid file."""

        if not self.filepath00.endswith(".grd") or not self.filepath45.endswith(".grd"):
            raise ValueError(
                "Error in BeamGrid.__post_init__: The files are not GRASP grid files."
            )
        with open(self.filepath00, "r") as fi:
            while True:
                line = fi.readline()
                if line[0:4] == "++++":
                    break
            ...
            self.ny = int(line[1])
            self.klimit = int(line[2])
            self.amp00 = np.zeros((self.ncomp, self.nx, self.ny), dtype=complex)
            ...
            for j in range(self.ny):
                if self.klimit == 1:
                 ...
                 # Obtain phi_rad, which will be used to perform Ludwig-3 to theta-phi basis conversion
                phi_rad = np.deg2rad(
                    self.ys + (j) * (self.ye - self.ys) / (self.ny - 1)
                )
                ...
                for i in range(is_val, in_val + 1):
                    ...
                    self.amp00[0, i - 1, j] = float(line[0]) + float(line[1]) * 1j
                    self.amp00[1, i - 1, j] = float(line[2]) + float(line[3]) * 1j

                    # Convert Ludwig-3 to theta-phi basis
                    c = self.amp00[0, i - 1, j]
                    x = self.amp00[1, i - 1, j]
                    self.amp00[0, i - 1, j] = c * np.cos(phi_rad) + x * np.sin(
                        phi_rad
                    )
                    self.amp00[1, i - 1, j] = -c * np.sin(phi_rad) + x * np.cos(
                        phi_rad
                    )

        with open(self.filepath45, "r") as fi:
            while True:
                line = fi.readline()
                if line[0:2] == [str(self.nx),str(self.ny)]
                    break
    
            self.amp45 = np.zeros((self.ncomp, self.nx, self.ny), dtype=complex)
            ...
            for j in range(self.ny):
                ...
                for i in range(is_val, in_val + 1):
                    ...
                    self.amp45[0, i - 1, j] = float(line[0]) + float(line[1]) * 1j
                    self.amp45[1, i - 1, j] = float(line[2]) + float(line[3]) * 1j

                    # Convert Ludwig-3 to theta-phi basis
                    c = self.amp45[0, i - 1, j]
                    x = self.amp45[1, i - 1, j]
                    self.amp45[0, i - 1, j] = c * np.cos(phi_rad) + x * np.sin(
                        phi_rad
                    )
                    self.amp45[1, i - 1, j] = -c * np.sin(phi_rad) + x * np.cos(
                        phi_rad
                    )
    ...
    def to_polar(self, copol_axis="x") -> BeamPolar:
        ...
        beam_polar0 = BeamPolar(
            nphi, ntheta, theta_rad_min, theta_rad_max, self.filename00 #FIXME: check filename
        )

        beam_polarC = BeamPolar(
            nphi, ntheta, theta_rad_min, theta_rad_max, self.filename00 #FIXME: check filename
        )

        beam_polarS = BeamPolar(
            nphi, ntheta, theta_rad_min, theta_rad_max, self.filename00 #FIXME: check filename
        )
        ...
        if self.icomp == 3:
            ...
            modc002 = np.abs(c00) ** 2
            modx002 = np.abs(x00) ** 2
            acax00s = c00 * np.conj(x00)

            modc452 = np.abs(c45) ** 2
            modx452 = np.abs(x45) ** 2
            acax45s = c45 * np.conj(x45)

            beam_polar0.stokes[0, :, :] = modc002 + modc452 + modx002 + modx452
            beam_polar0.stokes[1, :, :] = sign * (modc002 + modc452 - modx002 - modx452)
            beam_polar0.stokes[2, :, :] = sign * 2.0 * np.real(acax00s + acax45s)
            beam_polar0.stokes[3, :, :] = 2.0 * np.imag(acax00s + acax45s) # FIXME: check!!!

            beam_polarC.stokes[0, :, :] = modc002 - modc452 + modx002 - modx452
            beam_polarC.stokes[1, :, :] = sign * (modc002 - modc452 - modx002 + modx452)
            beam_polarC.stokes[2, :, :] = sign * 2.0 * np.real(acax00s - acax45s)
            beam_polarC.stokes[3, :, :] = 2.0 * np.imag(acax00s - acax45s) # FIXME: check!!!

            beam_polarS.stokes[0, :, :] = sign * np.real(c00 * np.conj(c45) + x00 * np.conj(x45)) #FIXME: check sign and factor
            beam_polarS.stokes[1, :, :] = sign * np.real(c00 * np.conj(c45) - x00 * np.conj(x45)) #FIXME: check sign and factor
            beam_polarS.stokes[2, :, :] = sign * np.real(c00 * np.conj(x45) + x00 * np.conj(c45)) #FIXME: check sign and factor
            beam_polarS.stokes[3, :, :] = 2.0 * np.imag(acax00s - acax45s) #FIXME: check!!!
            if swaptheta:
                beam_polar0.stokes = beam_polar0.stokes[:, :, ::-1]
                beam_polarC.stokes = beam_polarC.stokes[:, :, ::-1]
                beam_polarS.stokes = beam_polarS.stokes[:, :, ::-1]
        elif self.icomp == 9: #FIXME: what happens to C and S beams?
            c = self.amp[1, :-1, :]
            modc2 = np.abs(c) ** 2
            beam_polar0.stokes[0, :, :] = modc2
            beam_polar0.stokes[1, :, :] = 0.0
            beam_polar0.stokes[2, :, :] = 0.0
            beam_polar0.stokes[3, :, :] = 0.0
            if swaptheta:
                beam_polar0.stokes = beam_polar0.stokes[:, :, ::-1]
        else:
            raise ValueError(
        ...
        return beam_polar0, beam_polarC, beam_polarS
